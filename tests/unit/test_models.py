...
async def test_create_user(db_session):
    user = User(username="testuser1", email="test1@example.com", hashed_password="hashed_password")
    db_session.add(user)
...
async def test_user_representation(db_session):
    ...
    assert str(user) == f"User(id={user.id}, username='testuser2', email='test2@example.com')"

@pytest.mark.asyncio
async def test_user_repository_create(db_session):
    user_repo = UserRepository(db_session)
    user_data = UserCreateInDB(username="newuser", email="newuser@example.com", hashed_password="hashed_password")
...
async def test_user_repository_get_by_id(db_session):
    user_repo = UserRepository(db_session)
    user_data = UserCreateInDB(username="getbyid", email="getbyid@example.com", hashed_password="hashed_password")
    new_user = await user_repo.create(user_data)
...
async def test_user_repository_get_by_username(db_session):
    user_repo = UserRepository(db_session)
    user_data = UserCreateInDB(username="getbyusername", email="getbyusername@example.com", hashed_password="hashed_password")
...
async def test_user_repository_get_by_email(db_session):
    user_repo = UserRepository(db_session)
    user_data = UserCreateInDB(username="getbyemail", email="getbyemail@example.com", hashed_password="hashed_password")
    await user_repo.create(user_data)
...
async def test_user_repository_update(db_session):
    user_repo = UserRepository(db_session)
    user_data = UserCreateInDB(username="updateuser", email="updateuser@example.com", hashed_password="hashed_password")
    new_user = await user_repo.create(user_data)
    
    new_user.username = "updatedusername"
    updated_user = await user_repo.update(new_user)
...
async def test_user_repository_delete(db_session):
    user_repo = UserRepository(db_session)
    user_data = UserCreateInDB(username="deleteuser", email="deleteuser@example.com", hashed_password="hashed_password")
    new_user = await user_repo.create(user_data)
    
    await user_repo.delete(new_user)
...
async def test_user_repository_list(db_session):
    ...
    user_repo = UserRepository(db_session)
    for i in range(5):
        user_data = UserCreateInDB(username=f"listuser{i}", email=f"listuser{i}@example.com", hashed_password="hashed_password")
        await user_repo.create(user_data)
